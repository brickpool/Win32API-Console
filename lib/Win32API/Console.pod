...
Perl developers could only use the L<Win32::Console> module (ANSI only, some 
limitations). 
...
Further details can be found in the included POD or directly at MSDN. The API 
interface was modeled on the classic Windows API, so that the original 
documentation from Microsoft can be used without any problems. 
